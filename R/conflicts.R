#'
#' This function lists all the conflicts between packages in psylisten
...
psylisten_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", psylisten_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "psylisten_conflicts")
}

psylisten_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "psylisten_conflicts()"
  )
...
#' @export
print.psylisten_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(psylisten_conflict_message(x))
}
